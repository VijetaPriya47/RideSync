...
import (
	...
	"ride-sharing/services/user-auth-service/internal/infrastructure/events"
	grpcinfra "ride-sharing/services/user-auth-service/internal/infrastructure/grpc"
...
func main() {
	...
	auditConsumer := events.NewAuditConsumer(rmq, authSvc)
	if err := auditConsumer.Listen(); err != nil {
	...
	grpcinfra.NewGRPCHandler(srv, authSvc)

	go func() {
		log.Printf("Starting gRPC server user-auth-service on %s", grpcAddr)
	...
	<-ctx.Done()
	log.Println("Shutting down user-auth-service...")
	srv.GracefulStop()
}
